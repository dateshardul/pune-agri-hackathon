...
import { getWeather, getSoil, getForecast, type WeatherResponse, type SoilResponse, type ForecastResponse } from '../services/api';
...
export default function Dashboard({ lat, lon }: Props) {
  ...
  return (
    <section id="dashboard" className="dashboard accent-blue">
      ...
      {/* Simulation moved to Crop Simulation tab */}
      <div style={{ marginTop: '1rem', padding: '0.75rem 1rem', borderRadius: '8px', background: '#e8f5e9', fontSize: '0.9rem', color: '#2e7d32' }}>
        For crop simulations, yield predictions, and climate scenarios — go to the <strong>Crop Simulation</strong> tab.
      </div>
    </section>
  );
}
